...
# Green lab replication package For Incognito team
A smaller version of this repository can be found [here](https://github.com/andrempds99/GL_T_Incognito)

## Analysis
In this folder the R scripts used for the analysis are present. 
### Dataset
Here the CSV used for the analysis can be found.
### Figures
Inside figures, the generated graphs used for analysis are stored at Analysis and the ones present in the paper are stored at Finals.

## RawData
This folder contains the raw data gathered with the tools (BatteryStat and PerfumeJS). Also preparation data for the decision of the experiments and python scripts are stored here. The python scripts are used to aggregate the data from the tools into a single CSV.

### Preparation data
This data can be found in the preparation folder, one csv list some Web apps with the resource type that was found, the size in bytes and kilobytes, as well as the type of website.
On the second CSV and accompany XLSX file, all the possibilities of combinations that are possible for the resource hints and types are listed.

### Generated Web Apps
The generated Web apps are zipped in pieces to avoid the maximum size of Github files. Using zip program in Ubuntu, the following command recreates the original zip and then it can be unziped as normal.
```bash
zip -F webapps_noperfumejs.zip --out webapps.zip
```
To unzip the resulting file, the following command can be used.
```bash
unzip webapps.zip
```
More information can be found at [this link](https://superuser.com/questions/336219/how-do-i-split-a-zip-file-into-multiple-segments).

The websites present are before the application of PerfumeJS and after, so if any analysis without PerfumeJS is performed, then the adequate sample can be used.

## Programs
### WebAppGenerator
This program is written in C#.NET, it reads a file named `webapps.txt` where each line is a URL to a Web apps to generate different experiments. The file must be in the same folder as where the program is executed. To run the program, a .NET runtime must be present, please follow the installation guide at [this link](https://dot.net). After installing, it can be executed by running the following command.
```bash
dotnet run
```
### LCP_Reader
This program is written in JavaScript, it uses the [puppeteer](https://github.com/puppeteer/puppeteer) to control the Chrome browser and navigate to the [Google PageSpeed Insights](https://pagespeed.web.dev/) tool. This is used to retrieve the LCP of each of the Web apps to classify them as heavy or light.


### Data Aggregator
This experiment generates the Data for the performance metrics collected from the PerfumeJS. However, after the completion of the data, the data needs to be aggregated for all the files.

```bash
python3 GenerateData_V2.py <path_to_files>
```

The above script generates the file final_result.csv that aggregates the data and generate empty_directories.csv for the no data is generated by the android runner. Used for the re-run

# Experiment

To replicate the experiment, following setups are to be used

Step-1: We need to follow the Android Runner github to Install adb and the andoird runner setup(https://github.com/S2-group/android-runner/tree/master/AndroidRunner)

Step-2: For generating the Battery Stats data, the following commands needed to be executed.
```bash
python3 android-runner configs/batterystat.json
```

Step-3: For the performance metrics
```bash
python3 android-runner configs/perfume4.json
```

Both config files require monkey runner (<path_to_monkeyrunner>) and the power profile (<path_to_powerprofile>) to be filled in the respective variables.

## Experiment interactions
The script inside the interaction directory can be used for the experiment replication. The interaction happens in two places, just after the launch and while the page loads.
